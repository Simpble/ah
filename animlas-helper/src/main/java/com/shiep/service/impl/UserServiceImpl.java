...
public class UserServiceImpl implements IUserService {
    ...
    public boolean register(User user) {
        ...
        // 对密码进行MD5+salt 加密
        newUser.setPassword(Md5Utils.md5Hex(user.getPassword(), salt));
    ...
    public boolean updatePwd(Long userId, String oldPwd, String newPwd, String email) {
        ...
        // 根据盐来生成密码 与数据库中比较
        String pwd = Md5Utils.md5Hex(oldPwd, user.getSalt());
        if (!StringUtils.equals(pwd, user.getPassword())) {
            return false;
        }

        // 此时还要对新密码进行MD5加密处理, 此时盐就使用原来的吧
        String pwdInDb = Md5Utils.md5Hex(newPwd, user.getSalt());
    ...
    public boolean addUser(User addUser) {
        ...
        newUser.setSalt(salt);
        // 对密码进行MD5+salt 加密
        newUser.setPassword(Md5Utils.md5Hex(addUser.getPassword(), salt));
